...
class UserSerializer(serializers.ModelSerializer):
    """
    Serializer for the User model, handling validation,
    creation, and updating of user data.Includes additional logic
    for handling profile image uploads via Cloudinary.
    """
    ...
    class Meta:
        ...
        fields = ['id', 'username', 'password',
                  'password_again', 'profile_image', 'bio']
        extra_kwargs = {
    ...
    def get_profile_image(self, obj):
        if obj.profile_image:
            ...
            cloud_name = settings.CLOUDINARY_STORAGE.get(
                'CLOUDINARY_URL', '').split('@')[-1]
            return
            f"https://res.cloudinary.com/{cloud_name}/{obj.profile_image}"
        return None
    ...
    def create(self, validated_data):
        """
        Create a new user instance with optional
        ...
        if profile_image and not profile_image.content_type.startswith(
             'image/'):
            raise serializers.ValidationError(
        ...
        if profile_image:
            upload_result = cloudinary_upload(
             profile_image, resource_type="image")
    ...
    def update(self, instance, validated_data):
        ...
        including profile image replacement if provided.
        """
